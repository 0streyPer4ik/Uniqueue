...
@Options({
  ...
  watch: {
    'question.id_subject': function (newVal) {
      this.saveToStorage('id_subject', newVal);
    },
    'question.id_question_type': function (newVal) {
      this.saveToStorage('id_question_type', newVal);
    },
  }
})
class QuestionParserComponent extends Vue {
  ...
  question: Question = {
    id: 0,
    id_question_type: this.loadFromStorage('id_question_type') || 1,
    content: {
      description: "",
      answers: [
    ...
    id_subject: this.loadFromStorage('id_subject') || 1,
  };
  ...
  saveToStorage(key: string, value: any) {
    localStorage.setItem(`question_parser_${key}`, JSON.stringify(value));
  }

  loadFromStorage(key: string) {
    const saved = localStorage.getItem(`question_parser_${key}`);
  ...
  addSequenceAnswer() {
    this.sequenceData.answersType3.push({
      text: "",
      correctPosition: this.sequenceData.answersType3.length + 1,
    });
  }
  ...
  resetSequenceAnswers() {
    this.sequenceData.answersType3 = this.sequenceData.answersType3.map((answer, i) => ({
      ...answer,
      correctPosition: i + 1,
    }));
  }
  ...
  parseText() {
    ...
    if (this.question.id_question_type === 1) {
      ...
      if (lines.length < 8) {
        ElMessage({
          message:
            "Неверный формат входных данных для вопроса на соответствие! Минимум 8 строк требуется.",
          type: "error",
        });
        return;
      }
      ...
      this.question.content.description = ""
      const descrMatch = lines[0].match(/^\s*(?:\d+(?:\)|\.))?\s*(.+)\s*$/);
      console.log(descrMatch)
      if (descrMatch) {
        this.question.content.description = descrMatch[1];
      }
      ...
      while (i < lines.length) {
        ...
        if (line !== "") {
          ...
          const formulation = line.match(/^\s*\d[).]\s*(.*)\t([А-Яа-я])[).]\s*(.*)\s*$/);
          if (formulation) {
            this.matchingData.qDescription.push(formulation[1]);
            terms.push({
              letter: formulation[2],
              text: formulation[3],
            });
          }
        }
      ...
      if (pairCount !== terms.length || pairCount < 3 || pairCount > 5) {
        ElMessage({
          message:
            "Некорректное количество пар! Должно быть 3 или 4 формулировки и столько же терминов.",
          type: "error",
        });
        return;
      }
      ...
      for (let j = 0; j < pairCount; j++) {
        ...
        this.matchingData.answersType2[termIndex].matches =
          this.matchingData.qDescription[formulationIndex];
      }
      ...
      const correctAnswers = correctAnswerLine
        .split(":")[1]
        .split(",")
        .map((s) => s.trim());
      ...
      const correctAnswerLineIndex = lines.findIndex((line) =>
        line.startsWith("Правильный ответ:"),
      );

      this.question.content.description =
        lines
          .slice(0, correctAnswerLineIndex === -1 ? lines.length : correctAnswerLineIndex)
          .join("\n")
          .trim() || "";
      this.question.content.description = this.question.content.description.replace(/_+/, "_____");

      if (correctAnswerLineIndex !== -1) {
        this.question.content.correctAnswer = lines
          .slice(correctAnswerLineIndex)
          .join("/")
          .split("/")
          .map((answer) => answer.replace("Правильный ответ:", "").trim())
...
<template>
  <div>
    ...
    <el-radio-group v-model="question.id_question_type" @change="setQuestionType">
      <el-radio :value="1">Выбор одного ответа</el-radio>
      <el-radio :value="2">Соответствие</el-radio>
      <el-radio :value="3">Последовательность</el-radio>
      <el-radio :value="4">Один ответ</el-radio>
      <el-radio :value="5">Множественные ответы</el-radio>
    </el-radio-group>
    ...
    <el-input v-model="inputText" type="textarea" :autosize="{ minRows: 10, maxRows: 20 }" placeholder="Вставьте вопрос и ответы" style="width: 800px" />
    <el-button type="primary" @click="parseText" class="text_copy">Разобрать</el-button>

    <el-input v-model="question.content.description" type="textarea" style="width: 1080px" placeholder="Введите описание вопроса" />
    <br /><br />
    ...
    <div v-else-if="question.id_question_type === 3">
      <div v-for="(answer, index) in sequenceData.answersType3" :key="index">
        ...
        <el-input-number v-model="sequenceData.answersType3[index].correctPosition" :min="1" :max="sequenceData.answersType3.length" placeholder="Позиция" style="width: 20%; margin-right: 10px">
        </el-input-number>
    ...
    <div v-else-if="question.id_question_type === 4">
      <el-input v-model="question.content.correctAnswer" style="width: 1080px" placeholder="Введите правильный ответ" />
    </div>

    <!-- QuestionType5-->
    <div v-else-if="question.id_question_type === 5">
      <el-input v-model="question.content.correctAnswer" type="textarea" style="width: 1080px" placeholder="Введите варианты правильных ответов через строку" />
    </div>
