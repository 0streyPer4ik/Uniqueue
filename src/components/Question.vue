...
class QuestionComponent extends Vue {
  ...
  goEdit() {
    ...
    const routeData = this.$router.resolve({
      name: "edit",
      params: { id: this.qData.id.toString() },
    });
...
<template>
  <div class="question" :class="{ 'disabled-div': showResults }" :id="`question-${index}`" @dblclick="goEdit">
    <div v-if="qData.id_question_type == 1">
      ...
      <QuestionType1Component :qData="{ content: qData.content, showResults, id: qData.id }" @answer-submitted="handleAnswer" />
    </div>
    <div v-else-if="qData.id_question_type == 2" class="question_type_2">
      ...
      <QuestionType2Component :qData="qData" @answer-submitted="handleAnswer" />
    </div>
    ...
    <div v-else-if="qData.id_question_type == 3" class="question_type_3">
      ...
      <question-type-3-component :qData="qData" @answer-submitted="handleAnswer" />
    </div>
    ...
    <div v-else-if="qData.id_question_type == 4" class="question_type_4">
      ...
      <QuestionType4Component :qData="qData" @answer-submitted="handleAnswer" />
    </div>
    ...
    <div v-else-if="qData.id_question_type == 5" class="question_type_5">
      <div class="qustion5">
        <span class="text">Вопрос № {{ index + 1 }}</span>
        <span class="text_question">Напишите правильный вариант ответа</span>
        <QuestionType5Component :qData="qData" @answer-submitted="handleAnswer" />
      </div>
    </div>
  </div>
</template>
